...
char *directory = "/home/huangshan/Documents/measurement/SiPM_Measurements/20130611/hv_f1/";
char *fileName = "chip2_11_72.7";
...
//const int tBins = (tLimit[1]-tLimit[0])*1000/binTDC*0.5;
...
void PlotSpectrum()
{
	...
	TCanvas *c1 = new TCanvas("CTR measurement","CTR measurement results",40,20,1200,900);
	c1->Divide(3,2);
	c1->cd(1);
	T->Draw(Form("tot1>>TOT1(%d,%f,%f)",800,0.0,800.0));
	c1->cd(2);
	T->Draw(Form("tot2>>TOT2(%d,%f,%f)",800,0.0,800.0));
	...
	fprintf(ofile,"%.2f	%d	%.2f	%.2f	%d	%.2f	%.2f	%.2f	%.2f\n",nSigma,NLCorr,tLimit[0],tLimit[1],tBins,tot1Range[0],tot1Range[1],tot2Range[0],tot2Range[1]);
...
void DoCali(){
	...
	DoCorrection(ct,tot[0],nCount,nCharge,nCor,psName,tlimit,tBins,minTot,maxTot,totBins,fitMin,fitMax,fitf,nSigma,nTotMax);
}
...
void tdc_ctr()
{
	...
	cout<<"1. Read .dat file, convert to .root file and plot."<<"\n";
	cout<<"2. Read .root file and plot spectrum."<<"\n";
	cout<<"*******************************************************************"<<"\n";
